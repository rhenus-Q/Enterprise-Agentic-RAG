...
- **Cancellation adds no `stop_reason` value.** A cancelled run raises
  ...
  caller's decision to stop (ADR 017). Cancellation checks belong at the streamed node
...
# Run the web API (single process — run history is in-memory)
uv run uvicorn server.app:app --port 8000

# Run the web UI in dev mode (separate shell; /api is proxied to :8000)
