...
| `frontend/` | Vite + React + TypeScript UI (Ask / Documents / Runs). Plain CSS, no router/state/UI library. Renders only what the API reports — never an inferred default. `src/api/client.ts` selects the real or mock client with the module constant `USE_MOCKS` (no `VITE_*` env var); `src/api/types.ts` mirrors `server/schemas.py`. Colocated `*.test.tsx` critical-state suite runs under vitest. |
