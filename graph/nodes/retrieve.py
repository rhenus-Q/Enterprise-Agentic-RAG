...
def retrieve(state: GraphState):
    ...
    irrelevant documents) and records stop_reason so the caller (CLI or web
    API) can warn the user that local retrieval failed. In privacy mode the fallback is ignored
