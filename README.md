...
result.question                # the redacted question the graph actually ran
result.question_sha256         # SHA-256 of the original (pre-redaction) input
result.input_redacted          # True when redaction changed the input
```
...
Secret-shaped values in the question (OpenAI/Anthropic-style keys, GitHub
tokens, `api_key=` / `token=` / `password=` / `secret=` pairs) are replaced
with `[REDACTED]` at the engine boundary — *before* the question enters
`GraphState` — so no secret reaches the retriever, router, generator,
graders, or the outbound web-search query. The original input is used only to
compute `question_sha256`, so identical inputs still correlate across runs,
and is never stored in the result or in `raw_state`. This is best-effort
hygiene for debug artifacts, not a guarantee that every possible secret shape
is caught.
...
The trace contains `run_id`, `generated_at`, `node_path`,
`total_duration_ms`, `node_timings_ms`, `stop_reason`, the run counters
(nested under `counters`: `retries`, `tracked_llm_calls`, `web_search_count`,
...
(`trace_path=None`) no file is written.

The question is stored only as `question_redacted` — the redacted question
truncated to 80 characters — alongside `question_sha256` (hash of the
original input) and the `input_redacted` flag. That preview is still
user-supplied text, so store trace files accordingly; `traces/` is
