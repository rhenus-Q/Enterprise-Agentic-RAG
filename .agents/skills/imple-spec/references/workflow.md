...
Read the file from `the user's skill input`.
...
If the implementation depends on current LangChain / LangGraph / LangSmith / langchain-mcp-adapters / MCP integration behavior (for example LangChain retrievers, tools, document loaders, vector stores, OpenAI/Chroma/Tavily integration through LangChain, or other version-sensitive ecosystem APIs), consult the installed LangChain Docs MCP server (the configured `docs-langchain` MCP search tool / the configured `docs-langchain` MCP filesystem-docs tool) before editing code.
...
* Do not use LangChain Docs MCP for project-local rules already covered by `AGENTS.md`, source code, tests, evals, or roadmap docs.
...
active sandbox and approval permission (do not combine test directories into one `pytest`
...
generated-date text in the report body. Never infer or guess the date from model
knowledge, conversation history, Git history, existing reports, or existing
...
Then choose a collision-safe report path. Before writing, check candidate path
existence and select the first unused path in this order:
