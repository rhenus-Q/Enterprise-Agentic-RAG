...
This Skill is for project-level review reports. Documentation-drift reports under `docs/roadmap/docs-drift-review/` are project-level review reports (handled here), not Skill review reports.

Do not use this Skill for Skill review reports under:

* `docs/roadmap/skills-review/`

Use ``$apply-skill-review`` for Skill review reports instead.
...
If `the user's skill input` is empty, stop and ask the user for either:
...
Decide whether `the user's skill input` is an explicit path or a topic/focus query.
...
If `the user's skill input` is an explicit path:
...
If `the user's skill input` is not an explicit path:
...
Use targeted file listings only to discover actual existing files when filenames are
uncertain.
...
* documentation-drift and other report fixes may edit any tracked doc (e.g. `README.md`, `structure.md`, `docs/adr/**`, `evals/README.md`); validate a docs-only edit with `git diff --check` rather than a test run
