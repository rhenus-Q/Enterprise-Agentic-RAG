...
For any `??` (untracked) files shown by `git status --short`, read the file directly before assessing commit readiness. An untracked file may be a new corpus document, a generated file, a secrets file, or an accidental addition; it is invisible to `git diff` and must be reviewed explicitly.
...
* `[PASS]` for safe/expected
* `[WARN]` for needs attention
* `[BLOCK]` for a blocking issue
