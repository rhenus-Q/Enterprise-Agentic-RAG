---
name: eval-imple
description: Evaluate whether a proposed repository change is justified by current evidence, then implement and validate only the smallest correct change when warranted. Use when the requested change may be unnecessary, overly broad, or needs evidence-based scoping; no change is a valid outcome.
---
...
4. Apply this execution boundary: Implement only evidence-backed scope. Preserve behavior by default and honor the repository's explicit approval requirements for validation commands.
